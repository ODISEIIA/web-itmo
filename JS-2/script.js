...
function GenTable() {
    ...
    for (let i = 0; i < rows; i++){
        ...
        for (let j = 0; j < columns; j++){
            document.getElementById('tbody').children[i].innerHTML += '<td><form><input type="text" placeholder="Текст"><input type="button" value="Сохранить" onclick="WriteSmth(' + i + ', ' + j + ')"><form></td>';
        }
    }
...
function GenDefs(){
    document.getElementById('Def').children[2].innerHTML += '<form id="CF"><input type="text" id="CT" placeholder="Ширина таблицы" oninput="SwitchButtonText()"><select oninput="SwitchButtonText()"><option disabled selected>Border</option><option>hidden</option><option>dotted</option><option>dashed</option><option>solid</option></select><input id="CB" type="button" onclick="SwitchTableWidth()" value="Apply_Width_And_Border!"></form><br>';  
    document.getElementById('Def').children[3].innerHTML += '<input type="button" onclick="Gen_Header()" value="Заголовок"><br><br>';
    document.getElementById('Def').children[4].innerHTML += '<input type="button" onclick="Del_Table()" value="Удалить">';
}
...
function Gen_Header() {
    document.getElementById('table').innerHTML = '<caption>Заголовок<caption>' + document.getElementById('tbody').innerHTML;
}
...
function RandTheTable(){
    ...
    switch (R_Int){
        ...
        break;
        case 4:
            Rand_Cell.parentNode.innerHTML += '<form><input type="text" placeholder="Текст" value="SPbCTF{1337}"><input type="button" ///value="Сохранить" onclick="WriteSmth(' + Rand_Row + ', ' + Rand_Col + ')"><form>';
...
function Del_Table(){
    document.getElementById('Def').innerHTML = '<form id="form"><input name="rows" type="text" placeholder="Число строк"><input name="columns" type="text" placeholder="Число колоннок"><input type="submit" onclick="GenTable(),GenDefs()" value="Load_It!"></form>';
